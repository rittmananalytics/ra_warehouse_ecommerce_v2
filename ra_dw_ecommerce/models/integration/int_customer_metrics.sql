...
customer_order_metrics as (
    ...
    select
        ...
        stddev(order_total_price) as order_value_std_dev,
        ...
        -- Product diversity  
        count(distinct order_id) as unique_products_purchased, -- approximation from order data
...
customer_digital_metrics as (
    ...
    select
        ...
        -- Journey complexity
        avg(total_sessions_to_conversion) as avg_journey_complexity,
        -- Most common journey characteristics (using approx_top_count instead of mode)
        (select value from unnest(approx_top_count(journey_complexity, 1)) order by count desc limit 1) as most_common_journey_type,
        (select value from unnest(approx_top_count(conversion_timeline, 1)) order by count desc limit 1) as most_common_conversion_timeline
...
-- Customer Lifetime Value prediction components
...
final as (
    ...
    select
        ...
        dm.converted_orders,
        dm.most_common_journey_type,
        dm.most_common_conversion_timeline,
        ...
        -- Predicted CLV (simple calculation)
        case
            when clv.churn_probability < 1.0
            then om.avg_order_value * clv.predicted_annual_orders * (1 - clv.churn_probability) * 2  -- 2 year horizon
            else 0
